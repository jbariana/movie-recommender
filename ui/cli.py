...
from api.api import get_user_profile, save_rating
...
def cli():
    ...
    while True:
        ...
        if command == "profile":
            show_user_profile(user.getUserID())
            continue
...
def show_user_profile(user_id=1):
    profile = get_user_profile(user_id)

    print("\n===== USER PROFILE =====")
    print(f"Username: {profile['username']}")
    print(f"Average Rating Given: {profile['average_rating']}")
    print(f"Movies Rated: {len(profile['ratings'])}")

    print("\n-- Rated Movies --")
    if profile["ratings"]:
        for title, rating in profile["ratings"]:
    ...
    if profile["favorites"]:
        for (title,) in profile["favorites"]:
            print(f"★ {title}")
    else:
